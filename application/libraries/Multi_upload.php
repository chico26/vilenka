...
//		$CI =& get_instance();
	...
	function go_upload($field = 'userfile') {
		...
		foreach($_FILES[$field]['name'] as $key=>$value) {
			$start_index = $key;
			if ($_FILES[$field]['name'][$key]!=''){ $valid_flag=TRUE; break; }
		}
		
		// Is $_FILES[$field] set? If not, no reason to continue.
		if (!$valid_flag /*! isset($_FILES[$field]['name'][$start_index])*/)
		{
		...
		} else
		{
            ...
            $error_hold = array();
            $error_upload = FALSE;
			$iteration_limit = $num_files + $start_index;
		}
        ...
        for ($i=$start_index; $i < $iteration_limit; $i++) {
